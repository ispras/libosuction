...
static bool add_symbol (const char* func_name, tree symbol);
void dump_dynamic_symbol_calls (function* func, call_symbols *symbols);
void dump_node (cgraph_node *node);


static bool
parse_ref_1 (const char *func_name, tree ctor, 
	     auto_vec<tree, 10> *stack, unsigned HOST_WIDE_INT depth)
{
  ...
  if (depth == 0)
	...
	case COMPONENT_REF:
	  ...
	  FOR_EACH_CONSTRUCTOR_ELT (CONSTRUCTOR_ELTS (ctor), cnt, cfield, cval)
	    if (field == cfield)
	      return add_symbol (func_name, cval);
	  break;
  ...
  switch (TREE_CODE(t)) 
    ...
    case ARRAY_REF:
      FOR_EACH_CONSTRUCTOR_ELT (CONSTRUCTOR_ELTS (ctor), cnt, cfield, cval)
	if (!parse_ref_1 (func_name, cval, stack, depth - 1))
	  return false;
      break;
    ...
    case COMPONENT_REF:
      ...
      FOR_EACH_CONSTRUCTOR_ELT (CONSTRUCTOR_ELTS (ctor), cnt, cfield, cval)
	if (field == cfield)
	  return parse_ref_1 (func_name, cval, stack, depth - 1);
      break;
...
static bool
parse_ref (const char *func_name, tree *expr_p)
{
  tree *p, base, ctor;
  ...
  if (ctor == NULL || ctor == error_mark_node)
    return false;
  return parse_ref_1 (func_name, ctor, &expr_stack, expr_stack.length () - 1); 
}

static bool
add_symbol (const char* func_name, tree symbol)
{
  if (TREE_CODE (symbol) == ADDR_EXPR &&
      TREE_CODE (TREE_OPERAND (symbol, 0)) == STRING_CST)
    {
      ...
      auto symbols = dynamic_symbols.get (func_name);
...
{
  /* Constant string */
  if (add_symbol (sign->func_name, symbol)) 
    return true;
